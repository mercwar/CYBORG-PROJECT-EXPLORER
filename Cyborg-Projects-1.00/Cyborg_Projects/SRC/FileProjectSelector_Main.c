...
static void FP_Save(FP_STATE *st)
{
    ...
    for (int i = 0; i < count; ++i) {
        char buf[MAX_PATH];
        SendMessageA(st->hDst, LB_GETTEXT, i, (LPARAM)buf);
       char full[MAX_PATH];
lstrcpynA(full, st->ws->workingDir, sizeof(full));
PathAppendA(full, buf);
fprintf(f, "%s\n", full);
    ...
    FP_Log(st, "Saved project: %s", st->manifestPath);
}
...
static INT_PTR CALLBACK FP_DlgProc(HWND hDlg, UINT msg, WPARAM w, LPARAM l)
{
	...
	case ID_MENU_TOOLS_COPYPROJECT:FileProjectSelector_Main(hWnd, hEditLog, &g_ws);return 0;
